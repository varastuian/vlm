#!/usr/bin/env python3
"""
bit_cd_infer.py — change detection using a model pretrained on LEVIR-CD,
then Qwen VLM description of the detected changes.


USAGE:
  python bit_cd_infer.py --before before.png --after after.png --out results

  # skip the Qwen description step:
  python bit_cd_infer.py --before before.png --after after.png --no-describe

  # point at a different BIT_CD checkout instead of the vendored copy:
...
Notes:
  - Images are processed in 256x256 TILES (not a single whole-image resize) —
    ...
    straiSght down to 256x256 shrinks small buildings past the point the model
...
# vendor/bit_cd/ — no external clone needed. Resolved relative to this file
# so it works no matter what directory you run the script from.
...
def main():
    ap = argparse.ArgumentParser(description="LEVIR-CD pretrained change detection + VLM description")
    ...
    ap.add_argument("--bit-cd-repo", default=None,
                     help="Path to the BIT_CD model code + checkpoint. Defaults to the "
                          "vendored copy shipped in this repo (./vendor/bit_cd).")
